  ...
  execute(val) {
    ...
    const patterns = [
      ...
      // 見出し
      {
        pattern: /^(\*+)(.*)$/gm,
        replacement: (m, p1, p2) => {
          return `\n${p1.replace(/\*/g, "#")} ${textLevelSemanticsCheck(
            p2.trim()
          )}\n`;
        },
      },
      ...
      // テーブルに改行
      {
        pattern: /\n\|([\s|\S]*?)\|\n([^|])/g,
        replacement: (m, p1, p2) => {
          return `|${textLevelSemanticsCheck(p1)}|\n${p2}`;
        },
      },
      {
        pattern: /\n\|([\s|\S]*?)\|\n(?!\|)/g,
        ...
        replacement: (m, p1) => {
...

          {
            ...
            p1.split("\n").forEach((line) => {
              ...
              currentLevel = level;
              symbolCount[currentLevel] = symbolCount[currentLevel]
                ? symbolCount[currentLevel] + 1
                : 1;
              result +=
                textLevelSemanticsCheck(
                  line.replace("+ ", symbolCount[currentLevel] + ". ")
                ) + "\n";
            });
          }
      ...
      (() => {
        ...
        return (p1) => {
          if (
            ...
            return `{{PARAGRAPHS_REPACE_BACKLOG_TO_MARKDOWN-${
              paragraphs.length - 1
            }}}`;
          }
    ...
    // 引用範囲指定を戻す
    val = val.replace(
      /{{QUOTE_REPACE_BACKLOG_TO_MARKDOWN-(.*?)}}/g,
      (m, p1) => {
    ...
    // パラグラフを戻す
    val = val.replace(
      /{{PARAGRAPHS_REPACE_BACKLOG_TO_MARKDOWN-(.*?)}}/g,
      (m, p1) => {
